...
export function createUnusedPrizePoolWithdrawnEvent(
  marketId: BigInt,
  creator: Address,
  amount: BigInt
): UnusedPrizePoolWithdrawn {
  let unusedPrizePoolWithdrawnEvent =
    changetype<UnusedPrizePoolWithdrawn>(newMockEvent())

  unusedPrizePoolWithdrawnEvent.parameters = new Array()

  unusedPrizePoolWithdrawnEvent.parameters.push(
    new ethereum.EventParam(
      "marketId",
      ethereum.Value.fromUnsignedBigInt(marketId)
    )
  ...
  return unusedPrizePoolWithdrawnEvent
}
